...
class HnxGrabber(grabber.Grabber):
    ...
    def _explore_stock(self, stock_code: str) -> dict:
        ...
        url: str = f'{self._home}cophieu-etfs/chi-tiet-chung-khoan-ny-{stock_code}.html'
        ...
        for div in table.childNodes:
            ...
            if key.lower() == 'Ngày GD đầu tiên'.lower():
                date = val
            elif key.lower() == 'KLNY (Cổ phiếu)'.lower():
    ...
    def grab(self, start: int = 1, count: int = 10000) -> int:
        ...
        url: str = f'{self._home}vi-vn/cophieu-etfs/chung-khoan-ny.html'
        print(f'url:{url}')
        res: requests.Response = None
        try:
            res = requests.get(url)
